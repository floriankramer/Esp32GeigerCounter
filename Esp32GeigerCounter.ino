...
const char APPKEY[16] PROGMEM = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
// The following settings control the packet sending behaviour. Due to the overhead
// a package might have (at least 13 bytes) combining as many measurements as possible
// into a single packet combined with a high delay in between packets increases the
...
// The number of measurements transferred per packet. If this number gets to high
// and error will occur during compilation, as a lmic lora packet may only contain
// 51 bytes
// DEFAULT: 10
#define NUM_MEASUREMENTS 10

// This can be used to prevent sending of data if nothing was counted by the sensor yet.
// As this brakes the constant time assumption between measurements this should only be
// used (set to more than 0) if NUM_MEASUREMENTS is 1
// DEFAULT: 0
#define MIN_COUNT 0

// This defines the number of insiginificant bits that should not be transmitted.
// Increasing this value exponentially reduces the accuracy of the transmitted numbers,
// but also reduces the number of bytes required to transfer a given value.
// The default of 2 leads to all transmitted numbers being multiples of four. This introduces
// an error of at most 2, but allows for transmitting 512 in a sinle byte using variable byte
// encoding.
// DEFAULT: 2
#define DISCARDED_BITS 2

// Use at most 3 bytes to encode each measurement when sending the data via lora.
#define MAX_BYTES_PER_COUNT 3

// If the last packet has not yet been send properly new packets will land in a queue to
// be send as soon as possible. This determines the maximum number of packets that can be
// in that queue
...
// The worst-case packet size is MAX_BYTES_PER_COUNT bytes per measurement plus a 4 byte
// timestamp plus at least 13 bytes overhead (16 are used here to allow for some leeway).
#if NUM_MEASUREMENTS * MAX_BYTES_PER_COUNT + 4 + 16 > 51
...
// TODO(florian): in hal.cpp:80 the SPI.begin call needs to receive the correct pins for the
// communication to work (for ttgov1.2 they are 5, 19, 27, 18)
...
volatile unsigned int geiger_counts_index = 0;
volatile unsigned int geiger_counts[NUM_MEASUREMENTS];
...
const unsigned int MEASUREMENT_PERIOD_SECONDS = SEND_PERIOD / NUM_MEASUREMENTS;

const int PACKET_BUFFER_SIZE = 50;
unsigned char packet_buffers[PACKET_BUFFER_SIZE * PACKET_QUEUE_LENGTH];
unsigned int packet_lengths[PACKET_QUEUE_LENGTH];
unsigned int packet_queue_index = 0;
unsigned int packet_queue_length = 0;
...
void IRAM_ATTR onGeigerCounterSignal() {
  geiger_counts[geiger_counts_index]++;
}

// Remove the last package from the queue. Look at the next entry and check if a packet is queued.
void sendNextPacket() {
  packet_queue_index++;
...
void finishCountingCycle() {
  ...
  unsigned int i = (packet_queue_index + packet_queue_length) % PACKET_QUEUE_LENGTH;
  ...
  ostime_t ms = osticks2ms(now);

  // write the current timer in ms to the buffer in network byte order
  buffer[offset] = ms & 0xFF;
  buffer[offset + 1] = (ms >> 8) & 0xFF;
  buffer[offset + 2] = (ms >> 16) & 0xFF;
  buffer[offset + 3] = (ms >> 24) & 0xFF;
  offset += 4;

  // Iterate over all measurements of the last cycle
  for (unsigned int i = 0; i < NUM_MEASUREMENTS; i++) {
    // discard the least significant DISCARDED_BITS bits
    unsigned int m = geiger_counts[i] >> DISCARDED_BITS;
  ...
  *length = offset;
  ...
  if (packet_queue_length == 1) {
    // No other packets were queued, the package sending needs to be initialized.
    // If packet_queue_length was greater than one another packet is waiting to be
    // transfered by lmic and sendNextPacket will be called as soon as that packet
    // was send.
...
void enableGeigerInterrupt() {
  attachInterrupt(digitalPinToInterrupt(GEIGER_PIN), onGeigerCounterSignal, FALLING);
}
